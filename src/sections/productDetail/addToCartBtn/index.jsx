...
const AddToCartBtn = ({className, listProduct, session: sessionDefault}) => {
  ...
  useEffect(() => {
    if (sessionDefault) return
    async function myFunction() {
      const sessionRefresh = await getSession().then((res) => res)
      return setSession(sessionRefresh)
    }
    myFunction()
  }, [])
